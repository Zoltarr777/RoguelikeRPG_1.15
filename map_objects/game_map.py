...
from item_functions import cast_confusion, cast_fireball, cast_lightning, heal, cast_sleep, cast_sleep_aura, health_talisman_sacrifice, cast_mind_control, necromancy, recover_mana, cast_magic, burn, shoot_arrow, poison
...
class GameMap:
		...
		for r in range(max_rooms):
			...
			for other_room in rooms:
				...
				self.place_entities(new_room, entities, orc_tile, healing_potion_tile, scroll_tile, 
					...
					floor_tile, long_bow_tile, arrow_tile)
	...
	def place_entities(self, room, entities, orc_tile, healing_potion_tile, scroll_tile, troll_tile, stairs_tile, 
		...
		chestplate_tile, leg_armor_tile, helmet_tile, amulet_tile, floor_tile, long_bow_tile, arrow_tile):
		...
		monster_chances = {
			...
			'trap': from_dungeon_level([[25, 3]], self.dungeon_level)
			}
		...
		item_chances = {
			...
			'burn_potion': from_dungeon_level([[10, 5]], self.dungeon_level),
			'poison_potion': from_dungeon_level([[10, 5]], self.dungeon_level),
			'long_bow': from_dungeon_level([[10, 3]], self.dungeon_level), #10, 3
		...
		for i in range(number_of_items):
			...
			if not any([entity for entity in entities if entity.x == x and entity.y == y]):
				...
				if item_choice == 'healing_potion':
					...
					item_component = Item(use_function=poison, amount=5, turns=10, gold=5)
